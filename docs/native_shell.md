...
Debug builds normally start the Python development launcher. Release jobs build a target-native PyInstaller onedir runtime at `apps/desktop/src-tauri/binaries/dairr-backend/`, and Tauri bundles that directory as a resource. Generated runtime contents are ignored and the repository contains no sidecar executable. PyInstaller does not cross-compile, so each macOS architecture and Windows x64 must build on its corresponding native runner; the packager rejects a mismatched real-build target. Run the sidecar build and `--check-runtime` before building an artifact. On Windows, Tauri applies `CREATE_NO_WINDOW` when spawning the console executable, retaining redirected logs without flashing a console window.
